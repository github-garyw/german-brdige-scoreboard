            ...
            <template v-slot:default>
                <thead>
                    <tr>
                        ...
                        <th v-for="player in players" :key="player.key" class="text-left" v-bind:class="{highlight: isHighlight(player.key)}">
                            <v-text-field :label=player.title class="centered-input" :value=player.name />
                        </th>
                        <th style='{padding: 0px}'>
                            <template v-if="isSetNameStep() === true">
                                <v-btn color="primary" @click="onNameSet" x-small>
                                    Set
                                </v-btn>
                            </template>
                        </th>
                ...
                <tbody>
                    <tr v-for="round in rounds" :key="round.round" v-bind:class="{highlight: isCurrentRoundIdx(round.round)}">
                        <td>
                            {{ round.round + 1}}
                        </td>
                        <td v-for="player in players" :key="player.key" v-bind:class="{highlight: isHighlight(player.key)}">
                            <span>
                        ...
                        <td>
                            <template v-if="isBidding(round.round) === true">
                                <v-btn color="primary" @click="onBid" x-small>
                                    Bid
                                </v-btn>
                            </template>
                            <template v-if="isGameStart(round.round) === true">
                                <v-btn color="primary" @click="onGameDone" x-small>
                                    Next
                                </v-btn>
                            </template>
                        </td>
...
export default class Scoreboard extends Vue {
    ...
    onBid() {
        console.log(`onBid`);
        let round = this.rounds[this.currentRoundIdx];
        round.results = [...round.bids];
    ...
    isHighlight(player: number) {
        let cur = this.currentRoundIdx % 4;
        return player == cur;
    }
...
table__wrapper > table > thead > tr > th !important {
    padding-left: 8px;
}
