                ...
                <tbody>
                    <tr v-for="round in rounds" :key="round.round">
                        ...
                        <td v-for="player in players" :key="player.key">
                            <span>
                                <NumberCell 
                                    :value.sync=rounds[round.round].bids[player.key] 
                                    :editing=isBidding(round.round)></NumberCell>
                                    
                                <span v-show=!isBidding(round.round)>/</span>

                                <NumberCell v-show=!isBidding(round.round) 
                                    :value.sync=rounds[round.round].results[player.key] 
                                    :editing=isGameStart(round.round)></NumberCell>

                                <span v-show=!isCurrentRoundIdx(round.round)>/</span>

                                <NumberCell v-show=!isCurrentRoundIdx(round.round) 
                                    :value.sync=rounds[round.round].cumScores[player.key] 
                                    :editing=false></NumberCell>
                            </span>
                        </td>
                        <td>
